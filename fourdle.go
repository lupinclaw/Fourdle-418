...
func login(email, password string) error {
	...
	if err != nil {
		return errors.New("Entered incorrect password")
	}
...
func editAccount(email string, password string) error {
	log.Printf("[INFO] [%v] Attempting to edit account.\n", email)
	...
	// update the table
	_, err = db.Exec("update User set Email = $1, PasswordSaltAndHash = $2 where Email = $1;", email, string(hash[:]))
	if err != nil {
		return err
	}
	log.Printf("[INFO] [%v] Updated user.\n", email)
	return nil
}
...
func main() {
	...
	http.HandleFunc("/edit-account", func(resp http.ResponseWriter, req *http.Request) {
		...
		if err = editAccount(email, password); err != nil {
			log.Printf("[INFO] [%v] Failed to edit account: %v\n", email, err)
			resp.WriteHeader(http.StatusBadRequest)
			io.WriteString(resp, err.Error())
			return
		}
